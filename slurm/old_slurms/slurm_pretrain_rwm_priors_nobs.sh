#!/bin/bash
#SBATCH --job-name=rwm-prior-noboot
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --cpus-per-task=18
#SBATCH --time=24:00:00
#SBATCH --mem=64G
#SBATCH --output=/gpfs/work4/0/prjs0951/Giacomo/robotic_world_model/logs/slurm/rwm-prior-noboot-%j.out
#SBATCH --error=/gpfs/work4/0/prjs0951/Giacomo/robotic_world_model/logs/slurm/rwm-prior-noboot-%j.err

# =============================================================================
# RWM with Randomized Priors + NO BOOTSTRAPPING: Ensemble Pretraining
# =============================================================================
# This script trains an ensemble world model with RANDOMIZED PRIORS but
# WITHOUT BOOTSTRAPPING.
#
# Hypothesis: With randomized priors providing diversity, bootstrapping
# (training each ensemble member on random data subsets) may be unnecessary.
# Removing bootstrapping could improve data efficiency since all ensemble
# members see all data.
#
# Comparison experiment:
#   - This run: Priors + No Bootstrap (all members see all data)
#   - Other run: Priors + Bootstrap (each member sees random subset)
#
...
#
# Usage:
#   sbatch slurm_pretrain_rwm_priors_no_bootstrap.sh
# =============================================================================
...
# Bootstrap: DISABLED for this experiment
BOOTSTRAP="False"
...
RUN_NAME="pretrain-ensemble-prior${PRIOR_SCALE}-noboot"
# =======================================================
...
echo "========================================"
echo "RWM with Randomized Priors + NO BOOTSTRAPPING"
echo "========================================"
...
echo ""
echo "========================================"
echo "Random Priors (No Bootstrap) Pretraining Completed"
...
if [ $EXITCODE -eq 0 ]; then
    echo ""
    echo "SUCCESS: Random priors (no bootstrap) ensemble pretraining completed!"
    echo ""
    ...
    echo "  - Uncertainty metric: $UNCERTAINTY_METRIC"
    echo ""
    echo "Compare with bootstrapped version to evaluate:"
    echo "  - Data efficiency (loss curves)"
    echo "  - Ensemble diversity (epistemic uncertainty)"
    ...
    echo "  logs/slurm/rwm-prior-noboot-$SLURM_JOB_ID.err"
    echo ""
    echo "Common issues:"
